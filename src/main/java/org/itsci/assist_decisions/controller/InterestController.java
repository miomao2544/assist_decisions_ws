...
public class InterestController {
    ...
    @PostMapping("/lists/{username}")
    public ResponseEntity listInterestsByUsername(@PathVariable("username") String username){
        try{
            List<Interest> interests =  interestService.InterestsfindByUsername(username);
            return new ResponseEntity(interests, HttpStatus.OK);
        }catch (Exception e){
            return new ResponseEntity(null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
